...
import { trainAdaBoost, predictAdaBoost } from '../../lib/algorithms/ensemble'
...
export default function AdaBoost({ progress, rounds: roundsOverride }: Props) {
  const isScene = progress !== undefined
  const [playRounds, setPlayRounds] = useState(3)
  ...
  const allRounds = useMemo(
    () => trainAdaBoost(xs, labels, Math.max(1, activeRounds === 0 ? 1 : activeRounds)),
    [xs, labels, activeRounds]
  )

  const currentWeights = useMemo(() => {
    if (activeRounds === 0 || allRounds.length === 0) return Array(data.length).fill(1 / data.length)
    return allRounds[Math.min(activeRounds - 1, allRounds.length - 1)].weights
  }, [activeRounds, allRounds, data.length])

  const activeModel = activeRounds === 0 ? [] : allRounds.slice(0, activeRounds)
  ...
  const boundaryCells = useMemo(() => {
    ...
    for (let i = 0; i < GRID; i++) {
      for (let j = 0; j < GRID; j++) {
        ...
        const pred = predictAdaBoost(activeModel, [cx, cy])
        cells.push({
          x: xScale(cx - stepX / 2),
          y: yScale(cy + stepY / 2),
    ...
    return cells
  }, [activeModel, activeRounds, xDom, yDom, xScale, yScale, stepX, stepY])

  const maxWeight = Math.max(...currentWeights)
  const baseR = 3
    ...
    <div>
      {!isScene && (
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', marginBottom: '0.75rem', fontFamily: 'var(--font-display)', fontSize: 'var(--step--1)', flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--ink-soft)' }}>
            Rounds T
            <input
              type="range" min={1} max={10} value={playRounds}
      ...
      {isScene && (
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--step--1)', color: 'var(--ink-soft)', marginBottom: '0.5rem' }}>
          {activeRounds === 0 ? 'Equal weights' : `Round ${activeRounds}`}
        </div>
      )}

      <svg width={W} height={H} role="img" aria-label="AdaBoost boundary visualization">
      ...
      <div style={{ marginTop: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: 'var(--step--1)', color: 'var(--ink-soft)' }}>
        {activeRounds === 0 ? 'Equal weights — circle size = weight' : `T=${activeRounds} stumps · circle size ∝ weight`}
      </div>
    </div>
  )
}
